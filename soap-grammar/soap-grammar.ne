...
directiveSOAPinner -> [^,\n]:+ {% (d, l, reject) => {
  ...
  match = s.match(/import +from +(github\/[_\-a-zA-Z0-9\/]+)( *@([a-z0-9]+))?/)
  if (match) return Object.assign({repository: match[1]}, (match[3]) ? {version: match[3]} : {})
  ...
  match = s.match(/@([a-z][a-zA-Z0-9]*)/)
  if (match) return {parameterName: match[1], defaultValue: ''}
